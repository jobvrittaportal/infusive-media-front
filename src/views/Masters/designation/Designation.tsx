...
import { IDesignationTable } from './model';
import useDebounce from '../../../hooks/useDebounce';

const Designation = () => {
    const [designationsData, setDesignationsData] = useState<IDesignationTable>({
        designations: [],
        totalCount: 0
    });
    ...
    const handelEditStatus = async (row: any) => {
        const res = await fetchApi(
            `Designation/toggleStatus/${row.id}`,
            "PUT",
    ...
    return (
        <DesignationDiv>
            ...
            <CustomTable
                value={designationsData.designations}
