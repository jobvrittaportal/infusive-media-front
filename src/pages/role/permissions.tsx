...
import {Box,Checkbox,Flex,Grid,Text,Divider,} from "@chakra-ui/react";
...
const PermissionTree: React.FC<Props> = ({ value, onChange }) => {
  const toggle = (feature: string, action: PermissionAction) => {
    ...
    if (existing) {
      const updated = {
        ...existing,
        permissions: { ...existing.permissions, [action]: !existing.permissions[action],},
      };
  ...
  return (
    <Box >
      <Text fontWeight="semibold" mb={2}>Permissions</Text>
      <Box borderWidth="1px" borderRadius="md" overflow="hidden" gap={5}>
        <Flex bg="gray.50" py={2} px={4} justify="space-between" align="center" fontWeight="semibold">
            ...
            <>
              <Box key={feature.name} mb={3}>
                <Grid templateColumns="1fr 2fr" alignItems="center" gap={5}>
                  ...
                  <Flex justify="space-between">
                    {feature.permissions?.map((perm) => (
                      <Checkbox key={perm} isChecked={isChecked(feature.name, perm)} onChange={() => toggle(feature.name, perm)} className="font-poppins text_regular text_lg">
                        {perm}
                      </Checkbox>
                    ))}
                  </Flex>
                ...
                {feature.features?.map((sub) => (
                  <Grid key={sub.name} templateColumns="1fr 2fr" alignItems="center" mt={2} pl={6}>
                    <Text className="font-poppins text_medium text_md">{sub.label}</Text>
                    <Flex justify="space-between">
                      {sub.permissions?.map((perm) => (
                        <Checkbox key={perm} isChecked={isChecked(sub.name, perm)} onChange={() => toggle(sub.name, perm)} className="font-poppins text_regular text_lg">
                          {perm}
                        </Checkbox>
                      ))}
                    </Flex>
