...
import MyDiv from './lead.style';
import CustomToast from '../../../components/CustomToast';
import { useFetch } from '../../../hooks/useFetch';
import { useEffect, useState } from 'react';
import { CustomButton, CustomTable } from '../../../components';
import { ILead, ILeadTable, cards, defaultLead } from './model';
...
const Lead = () => {
  ...
  const loadLeads = async () => {
    const res = await fetchApi("Lead", "GET", null, `skip=${pagination.skip}&limit=${pagination.limit}&search=${search}`, '');
    if (res) {
      setLeadData(res);
    }
  };
  ...
  return (
    <MyDiv>
      <Box className="top-section">
        <Text className='font-poppins text_xxl text_semibold'>Lead</Text>
      ...
      <Flex justify='space-between' mt={6}>
        <Text className='font-poppins text_xxl text_semibold'>Lead List</Text>
