...
const Dashboard = () => {
  ...
  const showname = hasPermission('dashboard', 'read');
        ...
        <Flex justifyContent="space-between">
          <Box>
            <Text className="greeting text_4xl text_regular" textAlign="left">
              Hi, {showname && FullName(user)}
            </Text>
            <Text className="greeting text_md text_regular">
              Take a look your overview,
              <Text as="span" className="greeting text_lg text_medium">
                {' '}
                {formattedDate}
              </Text>
            </Text>
        ...
        >
          {dummyData.map((item, index) => (
            <Box key={index} className="stat-card" textAlign="left">
              <Text className="stat-title text_md text_medium" alignContent="left">
                {item.title}
              </Text>
